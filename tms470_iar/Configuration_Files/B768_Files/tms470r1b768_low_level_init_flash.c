...
//
// TITLE:  TMS470R1x low level init for the B768.
//
...
#include <TexasInstruments/iotms470r1b768.h>
...
int __low_level_init(void)
{
 // Temp2=(unsigned long int *)IRQV;
  ...
  // set size to 512KB, enable read-only protection
  MFBAHR0 = 0x00000000;
  MFBALR0 |= BLOCK_SIZE_512K + RONLY;
  ...
  // set size to 64KB
  MFBAHR2 = 0x00000040;
  MFBALR2 = BLOCK_SIZE_32K;

  // De activate RAM at 0x00400000 that is connected to memory select 3,
  // set size to 0KB
  MFBAHR3 = 0x00000040;
  MFBALR3 = 0x00008050;
...
//  SET FLASH WAIT STATES
  ...
  FMBAC1 = BNKPWR_ACTIVE+ BSTDBY_MAX;                // active
  FMBAC2 = PIPELINE_MODE + BSLEEP_MAX;               // 0 wait states
  FMMAC2 = BANK2_ENA + PMPPWR_ACTIVE + PSTDBY_MAX;   // bank1
  FMBAC1 = BNKPWR_ACTIVE+ BSTDBY_MAX;                // active
  FMBAC2 = PIPELINE_MODE + BSLEEP_MAX;               // 0 wait states
